...
public class NavigableAVL < E extends Comparable < E >>
  ...
  private ArrayList<E> storePreOrderTree =new ArrayList<>();
  private TreeSet<E>setElementTree;
  ...
  public int size() {
	...
	public SortedSet<E> headSet(E toElement) {
    	setElementTree=new TreeSet<>();
    	preOrderTraverseForHead(super.root,toElement);
		return setElementTree;
	}
	
	@Override
	public SortedSet<E> tailSet(E fromElement) {
		setElementTree=new TreeSet<>();
		preOrderTraverseForTail(super.root,fromElement);	
		return setElementTree;
	}
	
	@Override
	public Iterator<E> iterator() {
		preOrderTraverseForIter(super.root);
	...
	private class AVLIterator<E> implements Iterator<E> {
        ...
        public E next() {
        	if(!hasNext()) 	return null;
				...
				return (E) storePreOrderTree.get(currentIndex++);
			}	
        }
	}
	@SuppressWarnings("unused")
	private void preOrderTraverseForHead(Node<E>root,E toElement) {
		if(root==null)
			return;
		else {
			if(root.data.compareTo(toElement) < 0) {
				setElementTree.add(root.data);
			}
			preOrderTraverseForHead(root.left,toElement);
			preOrderTraverseForHead(root.right,toElement);
		}
	}
	@SuppressWarnings("unused")
	private void preOrderTraverseForTail(Node<E>root,E fromElement) {
		if(root==null)
			return;
		else {
			...
			preOrderTraverseForTail(root.left,fromElement);
			preOrderTraverseForTail(root.right,fromElement);
		}
	}
	private void preOrderTraverseForIter(Node<E> root) {
		if(root==null)
			return;
		else {
			storePreOrderTree.add(root.data);
			preOrderTraverseForIter(root.left);
			preOrderTraverseForIter(root.right);
		}
	}
